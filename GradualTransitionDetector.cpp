...
vector<FrameTransition> detectGradualTransitions_v2(vector<Mat> frames, float maxStdDev, int minLength,  ofstream& logFile) {
	...
	for (int i = 0; i < nrPotentialFades; i++) {
		...
		//  ecr_out > ecr_in (more edges vanish) for the fade-out transition
		...
		ecr_in = 0;
		ecr_out = -1;
		// ecr_out < ecr_in (more edges appear) for the fade-in transition
		prevFrame = frames[interm];
		end = interm + 1;
...
vector<FrameTransition> detectGradualTransitions_v3(vector<Mat> frames, float maxStdDev, int minLength, ofstream& logFile) {
	...
	for (int i = 0; i < nrPotentialFades; i++) {
		...
		// the standard deviation (= "contrast") gradually decreases in case of a fade-out transition
		start = interm - 1;
		currStdDev = refStdDev;
		prevStdDev = currStdDev;
		while (start >= 0 && prevStdDev >= currStdDev) {
		...
		// the standard deviation (= "contrast") gradually increases in case of a fade-in transition
		end = interm + 1;
		currStdDev = refStdDev;
		prevStdDev = currStdDev;
		while (end < nrFrames && currStdDev >= prevStdDev) {
